...
def create_decoder(layer_fts_global):
    with tf.variable_scope('decoder'):
        ...
        with tf.variable_scope('center1', reuse=tf.AUTO_REUSE):
            ...
            pc1_xyz = tf.reshape(pc1,(-1,64,3) )#64x3 center1
        ...
        with tf.variable_scope('center2', reuse=tf.AUTO_REUSE):
            ...
            pc2_xyz =tf_util.mlp_conv(pc2,[4*3],name='p2-2') #12x64 center2
        ...
        with tf.variable_scope('center3', reuse=tf.AUTO_REUSE):
            ...
            pc3_xyz =tf_util.mlp_conv(pc3,[512,256,64*3],name='p3-2') #6x64 center2
...
def generator(inputs, target_score, fea_in,is_training):
    with tf.variable_scope('generator') :
        features, avg_loss = create_encoder(inputs, target_score, is_training)
        fine,coarse2,coarse1= create_decoder(fea_in)
    return fine,coarse2,coarse1, features
...
def discriminator(layer_pts, is_training, reuse=None):
    with tf.variable_scope('discriminator') as scope:
        ...
        fea=tf_util.mlp(qrs, [128,256],name='dis_1')
        temp=tf.tile(tf.reduce_max(fea,axis=1,keepdims=True),[1,4096,1])
        fea=tf.concat([fea,temp],axis=-1)
