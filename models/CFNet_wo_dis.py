...
def sub_module(pts, fts_prev_RI, fts_prev_AE, rot_prev_AE, qrs, qrs_fea_AE, qrs_rot_AE, is_training, tag, K, D, P, C, use_fea_nn):
    ...
    with tf.variable_scope(tag+'fts_prev_RI', reuse=tf.AUTO_REUSE):
        if fts_prev_RI is not None:
            fts_prev_RI = tf.gather_nd(fts_prev_RI, indices, name=tag + 'fts_prev_RI')  
...
class Model:
    def __init__(self, inputs, gt1, gt2, gt, target_score, is_training):
        ...
        self.fine,self.coarse2,self.coarse1, self.avg_loss = self.create_encoder(inputs, target_score, is_training)
        self.outputs = self.fine
        self.loss, self.chloss= self.create_loss(self.coarse1,self.coarse2,self.fine ,gt1,gt2,gt,self.avg_loss)          
    ...
    def create_encoder(self,layer_pts, target_score, is_training):
        ...
        with tf.variable_scope('FC_hier', reuse=tf.AUTO_REUSE):
            with tf.variable_scope('f1-1', reuse=tf.AUTO_REUSE):
                fea1=tf.nn.relu(mlp(layer_fts_global, [1024]))
            with tf.variable_scope('f1-2', reuse=tf.AUTO_REUSE):
                fea2=tf.nn.relu(mlp(fea1, [1024]))
            with tf.variable_scope('f1-3', reuse=tf.AUTO_REUSE):
                fea3=mlp(fea2, [1024])
        ...
        with tf.variable_scope('center2', reuse=tf.AUTO_REUSE):
            with tf.variable_scope('p2-1', reuse=tf.AUTO_REUSE):
                pc2 = tf.nn.relu(mlp(fea2, [64*128]))
            pc2 = tf.reshape(pc2,(-1,64,128))
            pc2=tf.concat([pc2,pc1_xyz],axis=-1)
            with tf.variable_scope('p2-2', reuse=tf.AUTO_REUSE):
                pc2_xyz =mlp_conv(pc2,[4*3]) 
        ...
        with tf.variable_scope('center3', reuse=tf.AUTO_REUSE):
            with tf.variable_scope('p3-1', reuse=tf.AUTO_REUSE):
                pc3 = tf.nn.relu(mlp(fea3,[512*256]))
            pc3 = tf.reshape(pc3,(-1,256,512))
            pc3=tf.concat([pc3,pc2_xyz],axis=-1)
            with tf.variable_scope('p3-2', reuse=tf.AUTO_REUSE):
                pc3_xyz =mlp_conv(pc3,[512,256,64*3]) 
        ...
        return pc3_xyz,pc2_xyz,pc1_xyz,avg_loss

    
    def create_loss(self,coarse1,coarse2,fine ,gt1,gt2,gt,avg_loss):
        loss1=chamfer(coarse1, gt1)        
        loss2=chamfer(coarse2, gt2)
        loss3=chamfer(fine, gt)
        
        loss=0.01*avg_loss+0.1*loss1+0.2*loss2+loss3
